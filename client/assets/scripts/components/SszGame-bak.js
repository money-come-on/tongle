...
cc.Class({
    ...
    setPlayersPanel:function (data) {
        ...
        if(!this._playersPanel){
            var prefabMgr = this.getComponent("PrefabMgr"); 
            this._playersPanel = prefabMgr.ctorNodeByName("ssz_players_panel");
            this._playerCpts = this._playersPanel.getComponentsInChildren("SszPlayer");
            this.initSeat();
                        ...
                        if(child.name == "out"){
                            ...
                            rotate.children.forEach(function(anDao){
                                anDao.children.forEach(function(node){
                                    if(node.name == "dao"){
                                        ...
                                        node.setPosition(cc.v2( SEAT[playerSeat[panel._seatName]][child.name].score[anDao.name] ))
                                    }
                                })
                            })
                            upright.children.forEach(function(anDao){
                                anDao.children.forEach(function(node){
                                    if(node.name == "type"){
                                        node.anchorX = panel._seatName == "right"?1:0;
                                        node.setPosition(cc.v2( SEAT[playerSeat[panel._seatName]][child.name].type[anDao.name] ))
                                    }
                                })
                            })
                        }
    ...
    onReceiveIsAgreeGold:function(data){
        // console.log(data);
        ...
        if(topAgree){
            ...
            cc.module.utils.addClickEvent(btnConfirm,this.node,"SszGame","onclickAgreeGem",{seatInfo:data.info,code:"1"});
            cc.module.utils.addClickEvent(btnCancel,this.node,"SszGame","onclickAgreeGem",{seatInfo:data.info,code:"0"});
            name.string = data.name;
            allowIn.string = data.gold;
    ...
    clickSeat:function(e,data){
        ...
        // console.log(data);
        if(this._askTime>0){
            console.log(this._askTime,'已经点击过一次，等倒计时结束或等房主处理');
            return;
        }
        this._askTime = 180;
        cc.module.socket.send(SEvents.Ssz.SEND_PLAYER_ASK_SEAT,data,true);
        // cc.module.socket.send(SEvents.Ssz.SEND_PLAYER_SEAT,data,true);
    },
    ...
    setMatePanelCards:function (data,time) {
    ...
    update: function (dt) {
        ...
        if(this._timePanel && this._agreeTime && this._agreeTime>0){
            this._agreeTime -= dt;
            this.changeAgreeTime(this._agreeTime);
            if(this._agreeTime<=0){
                this._agreeTime = null;
                this._timePanel.active = false;
