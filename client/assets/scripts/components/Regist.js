cc.Class({
    ...
    onClickCheckedCode:function(){//点击验证
        ...
        console.log(codeNum);
        var patrn = /1[3|5|7|8|]\d{9}/;
        if(patrn.test(phoneNum) && codeNum && (password == repassword)){
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && (xhr.status >= 200 && xhr.status < 400)) {
                    ...
                    console.log(response);
                    if(response==1){
                        cc.module.socket.send(SEvents.SEND_PHONE_NUM,{phoneNum:phoneNum,userId:cc.module.self.userId,password:password},true);
                    }
                    else{
